...
class Vehicle:
    ...
    def car_type(self):
        choice = np.random.choice([0, 1], 1, p=[0.9, 0.1])
        if choice == 0:
            self.type = 0
        else:
            self.type = 1
    ...
    def update(self, lead, dt):
        ...
        if lead is None:
            ...
            if self.v < self.v_max - (20 + ((self.rijbaan - 1) * 15)) * factor:
                self.v += 1 * factor
            # 100 - 120 - 140
            elif self.v > self.v_max + ((self.rijbaan - 1) * 20) * factor and self.type == 0:
                self.v -= 1 * factor

            elif self.v > 85 * factor and self.type == 1:
                self.v -= 1*factor
        ...
        # -------------------------------
            ...
            if abs(lead.x - self.x) < self.v * self.t:
                if self.links == True and self.rijbaan != 3 and self.type == 0:
                    self.rijbaan += 1
                    self.v += 1 * factor
                    self.links = False
                    self.rechts = False
